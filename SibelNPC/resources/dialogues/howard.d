APPEND ~S!SibelJ~
  IF ~~ THEN BEGIN SibelGreetsHoward
    ...
    IF ~~ THEN EXTERN ~S!SHOWA~ GreetsSibel
  END
END
...
IF ~NumTimesTalkedToGT(0)~ THEN BEGIN 2ndMeeting
  SAY ~Hello again. The tree is still suffering, but a friend of Sibel is always welcome in my shop.~
  IF ~~ THEN GOTO MainMenu
END

// @TODO: second block '2ndMeeting' goes here when the tree is fixed
...
IF ~~ THEN BEGIN MainMenu
  ...
  // TODO: reenable this when the tree is fixed
  //+ ~NumItemsPartyGT("S!scoin",0)~ + ~I will buy a batch of high quality Arcanis leather with all my coin.~ + BuyHQ
